...
# ---------------------------------------------------------------------------
# Dev mode: bypass Replit OAuth entirely
# ---------------------------------------------------------------------------
if IS_DEV:
    def _ensure_dev_user():
        """Create or fetch the dev user and auto-login."""
        DEV_USER_ID = "dev-local-user"
        user = User.query.get(DEV_USER_ID)
        if not user:
            user = User(id=DEV_USER_ID, first_name="Pumposh", email="dev@local")
            db.session.add(user)
            db.session.commit()
        return user
