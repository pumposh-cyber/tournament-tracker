...
from models import Tournament, ChecklistItem
...
def tournament_detail(tournament_id):
    ...
    from utils import (
        get_distance_from_home, estimate_drive_time, get_weather_forecast,
        weather_code_to_description, weather_code_to_icon, parse_day_count,
        HOME_LOCATION,
    )
...
def add_tournament():
    if request.method == "POST":
        ...
        dates_display = request.form.get("dates_display", "").strip()
        if not dates_display:
            dates_display = parsed_date.strftime("%b %d, %Y")
        ...
        tournament = Tournament(
            ...
            hotel_recommendation=request.form.get("hotel_recommendation", "").strip(),
            hotel_link_notes=request.form.get("hotel_link_notes", "").strip(),
            car_rental_recommendation=request.form.get("car_rental_recommendation", "").strip(),
            hotel_booking_status=request.form.get("hotel_booking_status", "").strip(),
            car_booking_status=request.form.get("car_booking_status", "").strip(),
...
DEFAULT_CHECKLIST = {
    "Essential Volleyball Gear": [
        "Jersey(s) and extra uniform shirt",
        "Shorts / spandex (at least 2 pairs)",
        "Knee pads",
        "Volleyball shoes (game pair)",
        "Backup athletic shoes or slides",
        "Team socks (multiple pairs)",
        "Warm-up jacket or hoodie",
        "Hair ties / headband if needed",
        "Athletic tape, pre-wrap, ankle brace (if used)",
    ],
    "Hydration and Nutrition": [
        "Large water bottle (filled)",
        "Electrolyte packets or sports drink",
        "Easy carbs: bananas, granola bars, bagels",
        "Protein snacks: nuts, protein bars, yogurt",
        "Quick sugar for between sets (fruit snacks, dates)",
        "Packed lunch or plan for nearby food",
        "Small cooler with ice pack",
    ],
    "Recovery and Injury Prevention": [
        "Foam roller or massage ball",
        "Stretch band / resistance band",
        "Pain relief cream or spray",
        "Ice packs or instant cold packs",
        "Ibuprofen or basic first-aid meds",
        "Bandaids / blister care",
        "Extra socks to change between matches",
    ],
    "Travel and Comfort Items": [
        "Backpack or volleyball bag",
        "Phone + charger + portable battery",
        "Tournament schedule screenshot or printout",
        "Cash / card for food and parking",
        "Blanket or small chair for seating",
        "Change of clothes for after matches",
        "Toiletries for hotel",
        "Sleepwear",
    ],
    "Parent / Family Logistics": [
        "Directions to venue and parking info",
        "Hotel confirmation",
        "Emergency contacts",
        "Snacks and water for siblings",
        "Entertainment for downtime (book, tablet, headphones)",
    ],
}

CATEGORY_ICONS = {
    "Essential Volleyball Gear": "fa-volleyball",
    "Hydration and Nutrition": "fa-bottle-water",
    "Recovery and Injury Prevention": "fa-kit-medical",
    "Travel and Comfort Items": "fa-suitcase-rolling",
    "Parent / Family Logistics": "fa-users",
}
